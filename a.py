...
SQUARE_SIZE, CIRCLE_RADIUS = WITH // BOARD_COLS, WIDTH // (3 * BOAD_COLS)
...
def draw_lines():
    for i in range(1, BOARD_ROWS):
        pygame.draw.line(screen, BLACK, (0, i * SQUARE_SIZE), (WIDTH, i * SQURE_SIZE), LINE_WIDTH)
        pygame.draw.line(screen, BLACK, (i * SQUARE_SIE, 0), (i * SQUARE_SIZE, HEIGHT - 100), LINE_WIDTH)
...
def is_board_full():
    return all(boad[row][col] != 0 for row in range(BOARD_ROWS) for col in range(BOARD_COLS))
